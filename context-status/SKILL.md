...
description: |
  ...
  ATIVAR com: /context-status, status?, como está o contexto, quanto contexto resta,
  ...
  Controles: /context-status off auto · /context-status off · /context-status on.
---

# Context Status v1.0 — Guia Completo
...
**Detecção de compactação iminente:** Claude não percebe que o contexto está prestes a ser compactado. O Context Status não consegue avisar com antecedência — apenas registra o consumo estimado até o momento atual.
...
A skill Context Status opera de forma independente. Com o Context Guardian ativo, as duas se complementam: a Status traz visibilidade contínua, o Guardian traz a camada de ação.

### O que muda com o Guardian ativo

**Escalada automática:** quando a barra atinge estado crítico e a recomendação sobe para "Transferência Imediata", o Guardian executa o relatório `.md` e os Prompts de Retomada sem aguardar confirmação. Sem o Guardian, a Context Status apenas exibe o alerta — a ação continua sendo manual.
...
Emitir sempre como **card HTML via ferramenta de visualização** (garantindo que o container principal tenha `role="status"` e `aria-live="polite"`, ou `role="alert"` e `aria-live="assertive"` se a recomendação for Estado Crítico), no topo da resposta, seguido do conteúdo normal. O card tem quatro zonas:
...
```
Contexto   — ícone usuário: <path d="M8 2.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 1 0 0-5zM3 14c0-3.5 2-5 5-5s5 1.5 5 5" stroke-width="1.5" stroke-linecap="round"/>
...
Alertas    — ícone triângulo de alerta: <path d="M8 2 1.5 14h13zM8 7v3.5" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/><circle cx="8" cy="12.5" r="0.7" fill="currentColor"/>
...
- **Segurança (Mascaramento de Credenciais):** Nunca exponha chaves de API, senhas, tokens ou credenciais nas decisões-âncora ou outros campos do card. Mascare-os ativamente (ex: `sk-...`) se forem relevantes para o contexto, prevenindo o vazamento acidental de segredos no card renderizado.
- **Estabilidade de Layout (Mitigação):** Instrua o modelo a limitar o comprimento de campos dinâmicos (ex: projetos, decisões, arquivos) a no máximo 60 caracteres, truncando com `...`. Ao truncar, exija que envolva o texto em uma tag com o atributo `title` contendo o texto completo para preservar a acessibilidade (ex: `<span title="texto original">texto trunca...</span>`). Como não há camada de consumo para validação determinística, isso age como uma defesa de melhor esforço contra quebras visuais e exaustão no layout.
