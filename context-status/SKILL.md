...
- **Segurança (Prevenção de XSS):** Você deve obrigatoriamente higienizar e escapar caracteres HTML (`<`, `>`, `&`, `"`, `'`) de qualquer string de entrada (como nomes de projetos e domínios) antes de inseri-los no card HTML gerado, para evitar injeção de código malicioso.
